...
var spellWin = {
	...
	lang: {
		localize_ui: function() {
			...
			E$("btnOptionsCancel").value = E$("btnLangCancel").value = this.fetch("BTN_CANCEL");
			...
			E$("btnAllDone").value = E$("btnOptionsOK").value = E$("btnLangOK").value = this.fetch("BTN_OK");
			E$("btnShowOptions").value = this.fetch("BTN_OPTIONS");
			E$("btnResetDict").value = E$("btnResetAutoCorrect").value = this.fetch("BTN_RESET");
			...
			E$("fldLanguageMultipleLabText").innerHTML = this.fetch("LANGUAGE_MULTIPLE_INSTRUCTIONS");
			...
			E$("btnResetAutoCorrectLabText").innerHTML = this.fetch("OPT_PERSONAL_AUTO_CURRECT");
			if (spellWin.provider().AddWordsToDictionary == "SERVER") {
				E$("btnAddToDict").value = this.fetch("BTN_ADD_TO_DICT").replace("Personal", "Word")
			}
		},
	...
	tokens: {
		...
		appetureXHTML: function(i, j, green) {
			...
			for (var k = $$appeture.min; k <= $$appeture.max; k++) {
				...
				$$fragment = $$fragment == undefined ? "" : livespell.str.HTMLEnc("" + $$fragment);
		...
		appetureText: function(i, j) {
			...
			if ($$out.length > 2 && livespell.test.IE() && $$out.charCodeAt($$out.length - 1) == 13) {

				$$out = $$out.substr(0, $$out.length - 1) // IE Linebreak issue
			}
		...
		appeture: function(i, j, binsafe) {
			...
			for (var $$min = j; $$min > 0 && $$min > j - 15 && !found; $$min--) {
				if (livespell.test.eos(this.value[i][$$min]) || livespell.test.HTML(this.value[i][$$min]) || livespell.test.nl(this.value[i][$$min])) {
			...
			for (var $$max = j; $$max < doclen && $$max < j + 25 && !found; $$max++) {
				if (livespell.test.eos(this.value[i][$$max]) || livespell.test.HTML(this.value[i][$$max]) || livespell.test.nl(this.value[i][$$max])) {
					$$max--;
					found = true;
					break;
				}
	...
	actions: {
		registerclose: function() {
			...
			if (!spellWin.provider().ShowSummaryScreen && spellWin.provider().isUniPacked && spellWin.undo.bookmarks.length == 0) {
				var doret = false;
				srthtml = "<h2 class='spellnofify'>" + spellWin.lang.fetch("DONESCREEN_MESSAGE") + "</h2><br/>"
				try {
		...
		changeCurrent: function() {
			...
			if (spellWin.ui.editingNow) {
				spellWin.tokens.setAppeture(spellWin.currentDoc, spellWin.currentToken, E$("fldTextInput").value);
				spellWin.provider().onChangeWord(spellWin.tokens.getCurrent(), E$("fldTextInput").value);
			...
			if (oS.disabled) {
				...
				if (os_value == "__*REG*__") {
					...
					} else if (spellWin.provider().ServerModel.toLowerCase() == "asp.net" || spellWin.provider().ServerModel.toLowerCase() == "aspx") {
						a = "aspnetspell";
	...
	optionsMenu: {
		...
		show: function(flag) {
			...
			if (flag) {
				...
				E$("tDictCount").innerHTML = intPersonalEntries + " " + spellWin.lang.fetch("OPT_ENTRIES");
				...
				E$("tAutoCorrectCount").innerHTML = intPersonalAutoEntries + " " + spellWin.lang.fetch("OPT_ENTRIES");
				spellWin.ui.enable("btnResetAutoCorrect", intPersonalAutoEntries > 0);
				E$("optCaseSensitive").checked = spellWin.provider().CaseSensitive;
				E$("optAllCaps").checked = spellWin.provider().IgnoreAllCaps;
				E$("optNumeric").checked = spellWin.provider().IgnoreNumeric;
			}
		...
		set: function() {
			...
			spellWin.provider().setFormVals(E$("optCaseSensitive").checked, E$("optAllCaps").checked, E$("optNumeric").checked, E$("optSentence").checked)
		...
		for (var i = spellWin.currentDoc; i < this.tokens.value.length; i++) {
			for (var j = (i == spellWin.currentDoc ? spellWin.currentToken : 0); j < this.tokens.value[i].length; j++) {
				...
				if (this.tokens.isWord(i, j) && !spelltest && unknown) {
					livespell.cache.suglist = livespell.array.safepush(livespell.cache.suglist, this.tokens.value[i][j])
	...
	moveNext: function() {
		...
		suggestions = dsuggs;
		E$("TextShow").innerHTML = this.tokens.appetureXHTML(spellWin.currentDoc, spellWin.currentToken, isGreen);
		...
		if (!suggestions || !suggestions.length || suggestions[0] === "" || reason == "R") {
			...
			if (reason === "X" && spellWin.provider().isUniPacked) {
				suggestions = new Array("JavaScriptSpellCheck Trial", "Please register online", "javascriptspellcheck.com");
			}
			for (var i = 0; i < suggestions.length; i++) {
				if (reason === "X") {
		...
		switch (reason) {
			case "C":
				...
				break;
			case "R":
				E$("fldTextInputLab").innerHTML = spellWin.lang.fetch("REASON_REPEATED");
				oS.options.length = 0;
			...
			default:
				if (reason === "B") {
					E$("fldTextInputLab").innerHTML = spellWin.lang.fetch("REASON_BANNED");
					if (spellWin.provider().Strict) {
						this.ui.enable("btnIgnore,btnIgnoreAll,btnAddToDict", false)
					}
				} else if (reason === "E") {
					E$("fldTextInputLab").innerHTML = spellWin.lang.fetch("REASON_ENFORCED");
					this.ui.enable("btnIgnore,btnIgnoreAll,btnAddToDict", false)
				} else {
					E$("fldTextInputLab").innerHTML = spellWin.lang.fetch("REASON_SPELLING");
				}
			...
			for (var j = (i === this.currentDoc ? this.currentToken : 0); j < this.tokens.value[i].length; j++) {
				if (this.tokens.isWord(i, j)) {
					...
					if (spellWin.provider().CheckGrammar && !spellWin.tokens.startsSentence(i, j) && j > 1 && spellWin.tokens.value[i][j].toUpperCase() === spellWin.tokens.value[i][j - 2].toUpperCase()) {
						if (spellWin.tokens.value[i][j].toUpperCase() != spellWin.tokens.value[i][j].toLowerCase()) {
							spelltest = false;
							reason = "R";
						}
					}

					if (spelltest && spellWin.provider().CaseSensitive && spellWin.provider().CheckGrammar && spellWin.tokens.startsSentence(i, j) && livespell.test.lcFirst(word)) {
	...
	sendInitialAJAXRequest: function(binisrepeat) {
		...
		if (binisrepeat) {
			...
			livespell.ajax.send("WINSETUP", livespell.cache.wordlist[spellWin.providerID].join(livespell.str.chr(1)), spellWin.provider().Language, "0", spellWin.providerID)
		}
	...
	buildWordQue: function() {
		...
		for (var i = 0; i < this.tokens.value.length && memsize < memmax; i++) {
			for (var j = 0; j < this.tokens.value[i].length && memsize < memmax; j++) {
				var spelltest = true
				...
				if (this.tokens.isWord(i, j) && (!spelltest) && unknown) {
					...
					livespell.cache.wordlist[spellWin.providerID] = livespell.array.safepush(livespell.cache.wordlist[spellWin.providerID], this.tokens.value[i][j].toString())
				}
			}
		}
	...
	setTheme: function() {
		...
		if (strTheme.length) {
			E$("theme").setAttribute("href", "themes/" + strTheme + "/dialog-window.css")
		}
	},
	...
	ui: {
		...
		finished: function() {
			...
			E$("tDoc").innerHTML = spellWin.lang.fetch("DONESCREEN_FIELDS") + " " + spellWin.tokens.value.length;
			E$("tEdi").innerHTML = spellWin.lang.fetch("DONESCREEN_EDITS") + " " + spellWin.editcount;
			E$("tWrd").innerHTML = spellWin.lang.fetch("DONESCREEN_WORDS") + " " + spellWin.tokens.countWords();
		...
		setupLanguageMenu: function() {
			...
			for (var l = 0; l < livespell.cache.langs.length; l++) {
				E$("fldLanguage")[l] = new Option(livespell.cache.langs[l], livespell.cache.langs[l], false, spellWin.provider().Language == livespell.cache.langs[l]);
				E$("fldLanguageMultiple")[l] = new Option(livespell.cache.langs[l], livespell.cache.langs[l], false, spellWin.provider().Language == livespell.cache.langs[l]);
				if (spellWin.provider().Language === livespell.cache.langs[l]) {
					ffound = true;
				}
			}
			E$("fldLanguage")[l] = new Option(spellWin.lang.fetch("LANGUAGE_MULTIPLE"), spellWin.lang.fetch("LANGUAGE_MULTIPLE"), false, false);
			if (ffound !== true) {
				E$("fldLanguage").options.add(new Option(spellWin.provider().Language, spellWin.provider().Language, true, true), 0);
		...
		showEdit: function(flag) {
			...
			if (flag) {
				var oApp = spellWin.tokens.appetureText(spellWin.currentDoc, spellWin.currentToken)
...
window.iclose = function() {
	...
	if (spellWin.provider().WindowMode.length > 8 && spellWin.provider().WindowMode.toLowerCase().substr(0,9) == "jquery.ui") {
